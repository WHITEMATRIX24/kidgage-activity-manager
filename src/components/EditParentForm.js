...
const EditParentForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode) {
      try {
        const response = await axios.put(`https://main.d3781xttwrodcq.amplifyapp.com/api/personal/parent/${parentData._id}`, formData);
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`https://main.d3781xttwrodcq.amplifyapp.com/api/personal/parent/${parentData._id}`);
      setParentData(null);
      setFormData({
        username: '',
        email: '',
