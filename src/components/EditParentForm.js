...
const EditParentForm = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await axios.get('https://kidgage-admin-cxde.onrender.com/api/personal/search', {
        params: { query }
      });
      if (response.data) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEditMode) {
      try {
        const response = await axios.put(`https://kidgage-admin-cxde.onrender.com/api/personal/parent/${parentData._id}`, formData);
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`https://kidgage-admin-cxde.onrender.com/api/personal/parent/${parentData._id}`);
      setParentData(null);
      setFormData({
        username: '',
        email: '',
